...
class CRM_Banking_Form_StatementSearch extends CRM_Core_Form {
  private const VALUE_DATE_START_ELEMENT = 'value_date_start';
  private const VALUE_DATE_END_ELEMENT = 'value_date_end';
  private const BOOKING_DATE_START_ELEMENT = 'booking_date_start';
  private const BOOKING_DATE_END_ELEMENT = 'booking_date_end';
  private const MINIMUM_AMOUNT_ELEMENT = 'minimum_amount';
  private const MAXIMUM_AMOUNT_ELEMENT = 'maximum_amount';
  private const STATUS_ELEMENT = 'status_select';

  /**
   * Prefixes for the key of the custom key-value-pair elements for searching in the data_parsed JSON field.
   */
  private const CUSTOM_DATA_ELEMENTS_COUNT = 5;
  private const CUSTOM_DATA_KEY_ELEMENT_PREFIX = 'custom_data_key_name_';
  private const CUSTOM_DATA_KEY_LIST_ELEMENT_PREFIX = 'custom_data_key_list_';
  private const CUSTOM_DATA_VALUE_ELEMENT_PREFIX = 'custom_data_value_';
    ...
    foreach ($custom_parameter_list as $custom_parameter_name) {
      if (!empty($custom_parameter_name) && !empty($ajaxParameters[$custom_parameter_name])) {
        if (self::database_supports_json()) {
          ...
          $whereClauses[] = "AND LOWER(JSON_UNQUOTE(JSON_EXTRACT(tx.data_parsed, %{$firstParameterNumber}))) LIKE LOWER(%{$secondParameterNumber})";
      ...
      if (isset($render_cache[$account_id])) {
        ...
        for ($i = 1; $i < $accountReferenceListCount; $i += 2) {
          ...
          if (empty($account_data['name'])) {
            ...
            // one is enough, it's a known account
            break;
          }
        }
