...
class CRM_Banking_Page_Dashboard extends CRM_Core_Page {
  ...
  // phpcs:enable
    ...
    while ($results->fetch()) {
      ...
      if (!isset($account_week_data[$account_id][$week])) {
        $account_week_data[$account_id][$week] = [];
      }
    ...
    foreach ($account_week_data as $account_id => $account_data) {
      ...
      if (!isset($account_data['before'])) {
        $account_week_data[$account_id]['before'] = ['sum' => 0];
      }
  ...
  public function calculateStats($name, $where_clause, $payment_states) {
    $data = ['title' => $name, 'count' => 0, 'stats' => []];
