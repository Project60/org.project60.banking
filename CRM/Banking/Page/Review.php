...
class CRM_Banking_Page_Review extends CRM_Core_Page {
  ...
  // phpcs:enable
    ...
    // Check if closed (i.e. 'processed' or 'ignored').
   ...
   */
  public function getUnprocessedInfo($pid_list, $next_pid, $choices) {
   ...
   */
  public function execute_suggestion($suggestion_hash, $parameters, $btx_bao, $choices) {
    ...
    if ($suggestion) {
      ...
      if ($result) {
        if ($result === 're-run') {
          ...
          $transaction->commit();
          // NO SUCCESSFUL EXECUTION (because it's a re-run)
          return NULL;
        }
        else {
          // ALL GOOD:
