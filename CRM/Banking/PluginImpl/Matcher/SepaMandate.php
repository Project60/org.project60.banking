...
class CRM_Banking_PluginImpl_Matcher_SepaMandate extends CRM_Banking_PluginModel_Matcher {
  ...
  // phpcs:enable
    ...
    if (!isset($config->cancelled_contribution_status_id)) {
      $config->cancelled_contribution_status_id = NULL;
    }
    ...
    if (!isset($config->cancellation_default_reason)) {
      $config->cancellation_default_reason = E::ts('Unspecified SEPA cancellation');
    }
    ...
    // is a mapping of "contribution status id" => "penalty". If status not in list, no suggestion will be generated
    if (!isset($config->cancellation_status_penalty)) {
      $config->cancellation_status_penalty = ['1' => 0.0, '5' => 0.2];
    }
    ...
    if (!isset($config->cancellation_cancel_reason_default)) {
      $config->cancellation_cancel_reason_default = E::ts('Unknown');
    }
    ...
    if ($config->cancellation_cancel_fee && !empty($config->cancellation_cancel_fee_store)) {
      ...
      if (!isset($config->cancellation_value_propagation)) {
        $config->cancellation_value_propagation = [];
      }
    ...
    if (!isset($config->cancellation_create_activity_subject)) {
      $config->cancellation_create_activity_subject = E::ts('Follow-up SEPA Cancellation');
    }
  ...
  // phpcs:enable
    ...
    elseif ($mandate['entity_table'] == 'civicrm_contribution_recur') {
      ...
      if (empty($data_parsed['sepa_batch'])) {
        ...
        $contribution_id = CRM_Core_DAO::singleValueQuery($find_contribution_query, [
          1 => [$contribution_recur_id, 'Integer'],
          2 => [$data_parsed['sepa_batch'], 'String'],
        ]);
  ...
  // phpcs:enable
    $this->contribution = NULL;
    $cancellation_mode = $suggestion->getParameter('cancellation_mode');
    if (!empty($cancellation_mode)) {
      // CANCELLATION is an entirely different process...
      return $this->executeCancellation($suggestion, $btx);
    }
  ...
  public function visualize_match(CRM_Banking_Matcher_Suggestion $suggestion, $btx) {
    ...
    $mandate_reference = $suggestion->getParameter('mandate_reference');
    $cancellation_mode = $suggestion->getParameter('cancellation_mode');
    ...
    if (isset($result['id'])) {
      ...
      // add warnings, if any
      $smarty_vars['warnings'] = $suggestion->getEvidence();
      ...
      $smarty_vars['error'] = E::ts('Internal error! Cannot find contribution #') . $suggestion->getParameter('contribution_id');
    }
  ...
  public function visualize_execution_info(CRM_Banking_Matcher_Suggestion $suggestion, $btx) {
    ...
    if (empty($contact_id)) {
      ...
      $result = civicrm_api3('Contribution', 'get', ['id' => $suggestion->getParameter('contribution_id')]);
      if (isset($result['id'])) {
        $contribution = $result['values'][$result['id']];
        $contact_id   = $contribution['contact_id'];
        $smarty_vars['contact_id'] = $contact_id;
    ...
    $smarty_vars['cancellation_mode'] = $suggestion->getParameter('cancellation_mode');
    $smarty_vars['cancel_fee']        = $suggestion->getParameter('cancel_fee');
    $smarty_vars['cancel_reason']     = $suggestion->getParameter('cancel_reason');
