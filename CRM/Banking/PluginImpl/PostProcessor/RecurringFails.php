...
class CRM_Banking_PluginImpl_PostProcessor_RecurringFails extends CRM_Banking_PluginModel_PostProcessor {
  ...
  public function __construct($config_name) {
    ...
    if (!isset($config->cancel_reason_field)) {
      $config->cancel_reason_field = 'cancel_reason';
    }
    // also valid: contact, account, contact_account
    ...
    if (!isset($config->recurring_contribution_pi_ids)) {
      $config->recurring_contribution_pi_ids = $this->getSepaRecurringPaymentInstrumentIDs();
    }
    if (empty($config->contribution_success_status_ids)) {
      $config->contribution_success_status_ids = [1];
    }
    if (empty($config->contribution_failed_status_ids)) {
      $config->contribution_failed_status_ids = [3, 4, 7];
    }
  }
  ...
  protected function getEligibleContributions($context) {
    ...
    if (!empty($config->recurring_contribution_pi_ids) && is_array($config->recurring_contribution_pi_ids)) {
      $contribution_query['payment_instrument_id'] = ['IN' => $config->recurring_contribution_pi_ids];
    }
    if (!empty($config->contribution_failed_status_ids) && is_array($config->contribution_failed_status_ids)) {
      $contribution_query['contribution_status_id'] = ['IN' => $config->contribution_failed_status_ids];
    }
  ...
  // phpcs:enable
    ...
    if (!empty($contribution['contribution_recur_id'])) {
      ...
      $stats['contribution_recur'] = civicrm_api3('ContributionRecur', 'getsingle', ['id' => $stats['contribution_recur_id']]);
      ...
      // GROUP_CONTACT doesn't respect the defined order!
      $sequence = '';
  ...
  protected function getSepaRecurringPaymentInstrumentIDs() {
    if (self::$sepa_recurring_payment_instrument_ids === NULL) {
      self::$sepa_recurring_payment_instrument_ids = [];
