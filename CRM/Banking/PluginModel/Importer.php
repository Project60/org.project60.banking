...
abstract class CRM_Banking_PluginModel_Importer extends CRM_Banking_PluginModel_IOPlugin {
  ...
  protected array $_primary_btx_fields = [
    'version',
    'debug',
    'amount',
    'bank_reference',
  ...
  protected array $_compare_btx_fields = [
    'bank_reference' => TRUE,
    'amount' => TRUE,
    'value_date' => TRUE,
    'booking_date' => TRUE,
  ...
  protected ?CRM_Banking_BAO_BankTransactionBatch $_current_transaction_batch = NULL;
  protected array $_current_transaction_batch_attributes = [];
  protected int $_default_btx_state_id = 0;
   ...
   */
  abstract public function probe_file($file_path, $params);
   ...
   * @return TODO: data format?
   */
  abstract public function import_file($file_path, $params);
   ...
   * @return TODO: data format?
   */
  abstract public function probe_stream($params);
   ...
   * @return TODO: data format?
   */
  abstract public function import_stream($params);
  ...
  public function __construct($plugin_dao) {
    ...
    if (!isset($config->organisation_contact_ids)) {
      $config->organisation_contact_ids = '';
    }
    if (!isset($config->organisation_ba_ids)) {
      $config->organisation_ba_ids = '';
    }
  }
      ...
      if (preg_match('/^_.*NBAN_..$/', $key) || preg_match('/^_.*IBAN$/', $key)) {
        ...
        if (!isset($this->account_cache[$value])) {
          ...
          if (!empty($this->_plugin_config->organisation_ba_ids)) {
            $reference_search_params['ba_id'] = ['IN' => explode(',', $this->_plugin_config->organisation_ba_ids)];
          }
          ...
          if (!empty($potential_ba_ids) && !empty($this->_plugin_config->organisation_contact_ids)) {
            ...
            $ba_search_params = [
              'contact_id'   => ['IN' => explode(',', $this->_plugin_config->organisation_contact_ids)],
              'id'           => ['IN' => $potential_ba_ids],
              'return'       => 'id',
              'option.limit' => 0,
            ];
   ...
   */
  public function openTransactionBatch($batch_id = 0) {
    if ($this->_current_transaction_batch == NULL) {
      ...
      if ($batch_id) {
        ...
        $this->_current_transaction_batch_attributes['sum']
          = $this->_current_transaction_batch->ending_balance - $this->_current_transaction_batch->starting_balance;
      }
      else {
      ...
      $this->reportProgress(
        CRM_Banking_PluginModel_Base::REPORT_PROGRESS_NONE,
        E::ts('Internal error: trying to open BTX batch before closing an old one.'),
   ...
   */
  public function getCurrentTransactionBatch($store = TRUE) {
   ...
   * phpcs:disable Generic.Metrics.CyclomaticComplexity.TooHigh
   */
  public function closeTransactionBatch($store = TRUE) {
  // phpcs:enable
    if ($this->_current_transaction_batch != NULL) {
      if ($store) {
        ...
        if ($this->_current_transaction_batch->ending_balance) {
          $sum_in_bao = $this->_current_transaction_batch->ending_balance
            - $this->_current_transaction_batch->starting_balance;
          $deviation = $sum_in_bao - $this->_current_transaction_batch_attributes['sum'];
          $correct_value = $this->_current_transaction_batch->starting_balance
            + $this->_current_transaction_batch_attributes['sum'];
          if (abs($deviation) > 0.005) {
            // there is a (too big) deviation!
            // only log if it was set
            if ($this->_current_transaction_batch->ending_balance) {
              $this->reportProgress(
                0.0,
                sprintf(
                  E::ts('Adjusted ending balance from %s to %s!'),
                  $this->_current_transaction_batch->ending_balance,
                  $correct_value
                ),
        ...
        elseif ($this->_current_transaction_batch->starting_balance != NULL) {
        ...
        // set the dates
        if (!$this->_current_transaction_batch->starting_date
          && isset($this->_current_transaction_batch_attributes['starting_date'])
        ) {
          $this->_current_transaction_batch->starting_date
            = $this->_current_transaction_batch_attributes['starting_date'];
        }
        if (!$this->_current_transaction_batch->ending_date
          && isset($this->_current_transaction_batch_attributes['ending_date'])
        ) {
        ...
        $reference = str_replace(
          '{starting_date}',
          date($dateFormat, strtotime($this->_current_transaction_batch->starting_date)),
          $reference
        );
        $reference = str_replace(
          '{ending_date}',
          date($dateFormat, strtotime($this->_current_transaction_batch->ending_date)),
          $reference
        );
        ...
        $counter = 0;
        $query_params = [
          1 => [$final_reference, 'String'],
          2 => [$this->_current_transaction_batch->id, 'Integer'],
        ];
        ...
        while (CRM_Core_DAO::singleValueQuery($query_sql, $query_params)) {
          ...
          $query_params[1] = [$final_reference, 'String'];
        }

        $this->_current_transaction_batch->reference = substr($final_reference, 0, 64);
        $this->_current_transaction_batch->save();
      }
      elseif ($this->_current_transaction_batch_attributes['isnew']) {
      ...
      $this->reportProgress(
        CRM_Banking_PluginModel_Base::REPORT_PROGRESS_NONE,
        E::ts('Internal error: trying to close a nonexisting BTX batch.'),
        CRM_Banking_PluginModel_Base::REPORT_LEVEL_ERROR
      );
    }
  }
   ...
   */
  public function _updateTransactionBatchInfo($btx) {
    if ($this->_current_transaction_batch) {
      ...
      elseif (strtotime($attribs['starting_date']) > strtotime($btx['booking_date'])) {
      ...
      elseif (strtotime($attribs['ending_date']) < strtotime($btx['booking_date'])) {
      ...
      if ($this->_current_transaction_batch->currency && isset($btx['currency'])) {
        if ($this->_current_transaction_batch->currency != $btx['currency']) {
          $this->reportProgress(
            CRM_Banking_PluginModel_Base::REPORT_PROGRESS_NONE,
            E::ts('WARNING: multiple currency batches not fully supported'),
            CRM_Banking_PluginModel_Base::REPORT_LEVEL_WARN
          );
        }
      }
  ...
  public function checkAndStoreBTX($btx, $progress, $params = []) {
    ...
    if ($result['count'] > 0) {
      ...
      $this->reportProgress(
        $progress,
        E::ts('Duplicate BTX entry detected. Not imported!'),
        CRM_Banking_PluginModel_Base::REPORT_LEVEL_WARN
      );
      // RETURN FIRST ENTRY
    ...
    if (empty($btx['booking_date']) || strtotime($btx['booking_date']) === FALSE) {
      $this->reportProgress(
        $progress,
        E::ts('No valid booking date detected. Not imported!'),
        CRM_Banking_PluginModel_Base::REPORT_LEVEL_ERROR
      );
      return FALSE;
    }
    ...
    if (empty($btx['value_date']) || strtotime($btx['value_date']) === FALSE) {
      $this->reportProgress(
        $progress,
        E::ts('No valid value date detected. Not imported!'),
        CRM_Banking_PluginModel_Base::REPORT_LEVEL_ERROR
      );
      return FALSE;
    }
    ...
    if (isset($params['dry_run']) && $params['dry_run'] == 'on') {
      ...
      $log_entry = E::ts('DRY RUN: Did not create bank transaction (%1 on %2)', [
        1 => CRM_Utils_Money::format($btx['amount'], $btx['currency']),
