...
abstract class CRM_Banking_PluginModel_Importer extends CRM_Banking_PluginModel_IOPlugin {
  ...
  protected $_compare_btx_fields = array('bank_reference' => TRUE, 'amount' => TRUE, 'value_date' => TRUE, 'booking_date' => TRUE, 'currency' => TRUE);
   ...
   */
  protected function openTransactionBatch($batch_id = 0) {
    ...
    if ($this->_current_transaction_batch == NULL) {
      ...
      if ($batch_id) {
        ...
        $this->_current_transaction_batch->domain = $this->domain;
  ...
  function closeTransactionBatch($store = TRUE) {
    if ($this->_current_transaction_batch != NULL) {
      if ($store) {
        ...
        $counter = 0;
        $countAction = BankTransactionBatch::get()
          ->selectRowCount()
          ->setWhere([
            ['reference', '=', $final_reference],
            ['id', '!=', $this->_current_transaction_batch->id],
          ]);
        while ($countAction->execute()->countMatched() > 0) {
          $counter += 1;
          $final_reference = "DUPLICATE-{$counter}-{$reference}";
          $countAction->setWhere([
            ['reference', '=', $final_reference],
            ['id', '!=', $this->_current_transaction_batch->id],
          ]);
        }
   ...
   *
   * @phpstan-return array<string, mixed>|bool
   *   TRUE, if successful, FALSE if not, or a duplicate existing BTX as
   *   property array
   */
  function checkAndStoreBTX($btx, $progress, $params = array()) {
    ...
    $getAction = BankTransaction::get()->setLimit(1);
    foreach ($duplicate_test as $fieldName => $value) {
      $getAction->addWhere($fieldName, '=', $value);
    }
    try {
    ...
    if ($result->countFetched() > 0) {
      ...
      return $result->first(); // RETURN FIRST ENTRY
    }
    ...
    if (isset($params['dry_run']) && $params['dry_run'] == "on") {
      ...
      $btx['domain'] = $this->domain;
      try {
        $transaction = BankTransaction::create()->setValues($btx)->execute()->single();
      }
      catch (\CRM_Core_Exception $e) {
        $this->reportProgress(
          $progress,
          sprintf(E::ts('Error while storing BTX: %s'), $e->getUserMessage()),
      ...
      $log_entry = E::ts("Created BTX <b>%1</b> for <b>%2</b> on %3", [
        1 => $transaction['id'],
        2 => CRM_Utils_Money::format($transaction['amount'], $transaction['currency']),
        3 => CRM_Utils_Date::customFormat($transaction['booking_date'], CRM_Core_Config::singleton()->dateformatFull),
      ]);
      $this->reportProgress($progress, $log_entry);
      $this->_updateTransactionBatchInfo($transaction);
      return TRUE;
    }
  }
