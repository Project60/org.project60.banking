...
function _banking_civix_class_loader($class) {
  if ($class === 'CRM_Banking_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Banking_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Banking\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Banking\\') === 0) {
