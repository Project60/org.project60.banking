...
class CRM_Banking_PluginImpl_Matcher_DefaultOptions extends CRM_Banking_PluginModel_Matcher {
  ...
  public function match(CRM_Banking_BAO_BankTransaction $btx, CRM_Banking_Matcher_Context $context) {
    ...
    if ($config->manual_enabled) {
      if ($config->manual_show_always || $this->has_other_suggestions($btx)) {
        ...
        $btx->addSuggestion($manually_processed, 'manual');
      }
    }
    ...
    if ($config->ignore_enabled) {
      if ($config->ignore_show_always || $this->has_other_suggestions($btx)) {
        ...
        $btx->addSuggestion($not_relevant, 'ignore');
      }
    }
  ...
  function visualize_match( CRM_Banking_Matcher_Suggestion $match, $btx) {
    if (substr($match->hash, -6) === "manual") {
      // this is the 'manual' suggestion
      return $this->_plugin_config->manual_message."<br/>".$this->_plugin_config->manual_contribution.'<input id="default_manual_contribution" class="form-text ac_input" type="text" style="width: 12em;" placeholder="contribution id" autocomplete="off"/>';

    } else {
      return $this->_plugin_config->ignore_message."<br/>".$this->_plugin_config->ignore_contribution;
