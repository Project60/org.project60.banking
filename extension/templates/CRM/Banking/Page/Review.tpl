  ...
  <div align="right" class="clearfix" style="width: 100%;">
    {if $btxstatus.label != 'Processed'}
      ...
      {if isset($url_skip_forward)}
        <a href="#" onClick="execute_selected('{$url_execute}')" class="button button-disabled"><span title="Confirm and Continue"><div class="icon next-icon"></div>Confirm and Continue</span></a>
        <a href="{$url_skip_forward}" class="button"><span title="Skip"><div class="icon next-icon"></div>Skip</span></a>
      {else}
        <a href="#" onClick="execute_selected('{$url_execute}')" class="button button-disabled"><span title="Confirm and Exit"><div class="icon next-icon"></div>Confirm and Exit</span></a>
...
function execute_selected(url) {
  // TODO:
  var parameters = cj('input[name=selected_suggestion]:checked').val();
  location.href = url.replace("__execution_parameters__", parameters);
}
