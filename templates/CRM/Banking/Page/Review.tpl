...
{* This page is generated by CRM/Banking/Page/Review.php *}
  ...
  <div align="right" class="clearfix" style="width: 100%;">
    <button type="button" class="crm-button {if not $url_skip_back}disabled{/if}  ui-icon-seek-prev" onclick="location.href='{$url_skip_back}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Back{/ts}"><div class="icon previous-icon ui-icon-seek-prev disabled"></div>{ts domain='org.project60.banking'}Back{/ts}</button>

    {if $btxstatus.label != 'Processed' AND $btxstatus.label != 'Ignored'}
      <button type="button" id="analyseButton" onClick="analysePayment()" class="crm-button" title="{ts escape='htmlattribute' domain='org.project60.banking'}Analyse (again){/ts}"><div class="icon preview-icon ui-icon-refresh"></div>{ts domain='org.project60.banking'}Analyse (again){/ts}</button>
      {if isset($url_skip_forward)}
        <button type="button" onClick="execute_selected()" class="crm-button" title="{ts escape='htmlattribute' domain='org.project60.banking'}Confirm and Continue{/ts}"><div class="icon next-icon ui-icon-check"></div>{ts domain='org.project60.banking'}Confirm and Continue{/ts}</button>
        <button type="button" class="crm-button" onclick="location.href='{$url_skip_forward}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Skip{/ts}"><div class="icon next-icon ui-icon-seek-next"></div>{ts domain='org.project60.banking'}Skip{/ts}</button>
        {if $url_skip_processed}
        <button type="button" class="crm-button" onclick="location.href='{$url_skip_processed}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Skip Processed{/ts}"><div class="icon next-icon ui-icon-seek-next"></div>{ts domain='org.project60.banking'}Skip Processed{/ts}</button>
        {/if}
      {else}
        <button type="button" onClick="execute_selected()" class="crm-button" title="{ts escape='htmlattribute' domain='org.project60.banking'}Confirm and Exit{/ts}"><div class="icon next-icon ui-icon-check"></div>{ts domain='org.project60.banking'}Confirm and Exit{/ts}</button>
        <button type="button" class="crm-button" onclick="location.href='{$url_back}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Skip and Exit{/ts}"><div class="icon next-icon ui-icon-seek-next"></div>{ts domain='org.project60.banking'}Skip and Exit{/ts}</button>
      {/if}
    {else}
      <button type="button" id="analyseButton" onClick="analysePayment()" class="crm-button disabled" title="{ts escape='htmlattribute' domain='org.project60.banking'}Analyse (again){/ts}"><div class="icon preview-icon ui-icon-refresh"></div>{ts domain='org.project60.banking'}Analyse (again){/ts}</button>
      {if isset($url_skip_forward)}
        <button type="button" class="crm-button disabled" title="{ts escape='htmlattribute' domain='org.project60.banking'}Confirm and Continue{/ts}"><div class="icon next-icon ui-icon-check"></div>{ts domain='org.project60.banking'}Confirm and Continue{/ts}</button>
        <button type="button" class="crm-button" onclick="location.href='{$url_skip_forward}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Skip{/ts}"><div class="icon next-icon ui-icon-seek-next"></div>{ts domain='org.project60.banking'}Skip{/ts}</button>
        {if $url_skip_processed}
        <button type="button" class="crm-button" onclick="location.href='{$url_skip_processed}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Skip Processed{/ts}"><div class="icon next-icon ui-icon-seek-next"></div>{ts domain='org.project60.banking'}Skip Processed{/ts}</button>
        {/if}
      {else}
        <button type="button" class="crm-button disabled" title="{ts escape='htmlattribute' domain='org.project60.banking'}Confirm and Exit{/ts}"><div class="icon next-icon ui-icon-check"></div>{ts domain='org.project60.banking'}Confirm and Exit{/ts}</button>
        <button type="button" class="crm-button" onclick="location.href='{$url_back}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Skip and Exit{/ts}"><div class="icon next-icon ui-icon-seek-next"></div>{ts domain='org.project60.banking'}Skip and Exit{/ts}</button>
      {/if}
    {/if}
    {if $new_ui_enabled && $back_to_statement_lines}
      <button type="button" class="crm-button" onclick="location.href='{$url_back}'" style="float:right;" title="{ts escape='htmlattribute' domain='org.project60.banking'}Back{/ts}">
        <div class="icon back-icon ui-icon-arrowreturnthick-1-w"></div>{ts domain='org.project60.banking'}Back to statement lines{/ts}
      </button>
    {elseif $new_ui_enabled && !$back_to_statement_lines}
      <button type="button" class="crm-button" onclick="location.href='{$url_back}'" style="float:right;" title="{ts escape='htmlattribute' domain='org.project60.banking'}Back{/ts}">
        <div class="icon back-icon ui-icon-arrowreturnthick-1-w"></div>{ts domain='org.project60.banking'}Back to statements{/ts}
      </button>
    {else}
      <button type="button" class="crm-button" onclick="location.href='{$url_back}'" title="{ts escape='htmlattribute' domain='org.project60.banking'}Back{/ts}">
        <div class="icon back-icon ui-icon-arrowreturnthick-1-w"></div>{ts domain='org.project60.banking'}Back to transaction list{/ts}
      </button>
    {/if}
  </div>
